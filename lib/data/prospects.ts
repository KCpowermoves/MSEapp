...
export async function listAvailableProspects(opts: {
  ...
  return all
    ...
    .sort((a, b) =>
      (a.businessName || a.contactName).localeCompare(
...
/** Mark every remaining New prospect Used — the admin "clear list"
 *  action. Rows stay for the record; they just drop off the picker. */
export async function clearProspects(): Promise<number> {
  const all = await listAllProspects({ fresh: true });
  let cleared = 0;
  for (const p of all) {
